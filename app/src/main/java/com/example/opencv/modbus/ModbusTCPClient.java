...
public class ModbusTCPClient {
        ...
        synchronized (requesrlock) {
            try {
                ...
                if (retryCount < 3) { // 限制最多重试3次
                    if (Reconnect()) {
                        Log.d("fileaddr", "read reconnection successful");
                        // 重试时递增计数器
                        return readReg(startAddress, quantity, retryCount + 1);
                    } else {
                        disconnect();
                        throw new ModbusException(R.string.Reconnection_failed + e.getMessage());
                    }
                } else {
                    disconnect();
                    throw new ModbusException(R.string.Reconnection_failed_info + e.getMessage());
                }
            }
        }
    }
    ...
    public boolean Reconnect() {
        disconnect();
        Log.d("fileaddr", "进入重连(Reconnecting)");
        int MaxRetry = 3;
        int retryCount = 0;
        while (retryCount < MaxRetry) {
            ...
            try {
                ...
                if (isConnected.get()) {
                    Log.d("fileaddr", "重连成功(Reconnected successfully)");
                    return true;
                }
            } catch (ModbusException e) {
                Log.d("fileaddr", "Reconnection failed: " + e.getMessage());
            }
            retryCount++;
        }
        return false;
        ...
        synchronized (requesrlock) {
            try {
                ...
                if (retryCount < 3) { // 限制最多重试3次
                    if (Reconnect()) {
                        // 重试时递增计数器
                        Log.d("fileaddr", "write reconnection successful");
                        writeReg(startAddr, values, retryCount + 1);
                    } else {
                        disconnect();
                        throw new ModbusException("Reconnection failed: " + e.getMessage());
                    }
                } else {
                    disconnect();
                    throw new ModbusException("write exceeded the maximum retry count (3 times): " + e.getMessage());
                }
            }
        }
    }
        ...
        synchronized (requesrlock) {
            ...
            try {
                ...
                if (retryCount < 3) { // 限制最多重试3次
                    if (Reconnect()) {
                        // 重试时递增计数器
                        Log.d("fileaddr", "File reconnected successfully");
                    ...
                    disconnect();
                    throw new ModbusException("File exceeded the maximum retry limit (3 times): " + e.getMessage());
                }
            }
        }
    }
...
//            }
    ...
    public void onWriteFailed(Context context, String error) {
        Toast toast = Toast.makeText(context, error +"."+ R.string.CHECK_CONNECT, Toast.LENGTH_SHORT);
        toast.show();
    }
